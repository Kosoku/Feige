...
final class ViewController: UITableViewController {
    ...
    private lazy var diffableDataSource: DiffableDataSource = { [unowned self] in
        DiffableDataSource(tableView: self.tableView) { tableView, indexPath, item in
            tableView.dequeueReusableCellClass(UITableViewCell.self, for: indexPath)
                .also {
                    $0.contentConfiguration = $0.defaultContentConfiguration()
                        .also {
                            $0.text = item.title
                        }
                }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection sectionIndex: Int) -> UIView? {
        ...
        return tableView.dequeueReusableHeaderFooterViewClass(UITableViewHeaderFooterView.self)
            .also {
                $0.contentConfiguration = $0.defaultContentConfiguration()
                    .also {
